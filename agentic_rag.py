...
# Define domain lists for each country
include_domains_finland = [
    "migri.fi",
    "enterfinland.fi",
    "businessfinland.fi",
    "kela.fi",
...
4. **Citations (MANDATORY, NEVER OMIT)**:
   ...
   - **IMPORTANT**: For Internet search results, ONLY include URLs and information from these specific domains: {domains_finland}. Ignore any search results from other domains.
...
Answer: <|eot_id|><|start_header_id|>assistant<|end_header_id|>""",
    input_variables=["question", "context", "answer_style", "domains_finland"]
)
...
estonia_rag_prompt = PromptTemplate(
    template=r"""<|begin_of_text|><|start_header_id|>system<|end_header_id|>
            ...
            - Always base your answers on the provided context and answer style. If the context contains irrelevant text, say 'No information found'.
            ...
            - **IMPORTANT**: For Internet search results, ONLY include URLs and information from these specific domains: {domains_estonia}. Ignore any search results from other domains.
            ...
            Answer style: {answer_style}
            Answer: <|eot_id|><|start_header_id|>assistant<|end_header_id|>""",
    input_variables=["question", "context", "answer_style", "domains_estonia"]
)
...
def initialize_grading_llm(selected_grading_model):
    if "grader_llm" not in st.session_state or st.session_state.grader_llm.model_name != selected_grading_model:
        if "gpt-" in selected_grading_model:
            st.session_state.grader_llm = ChatOpenAI(
                model=selected_grading_model, temperature=0.0, max_tokens=4000)
...
def generate(state):
    ...
    # Get the domains based on selected country
    if st.session_state.selected_country == "Finland":
        domains = ", ".join(include_domains_finland)
        rag_chain = st.session_state.rag_prompt | st.session_state.llm | StrOutputParser()
    else:  # Estonia
        domains = ", ".join(include_domains_estonia)
        rag_chain = st.session_state.rag_prompt | st.session_state.llm | StrOutputParser()
    ...
    while len(tried_models) < len(model_list):
        try:
            ...
            # Update the RAG chain with the appropriate prompt
            if st.session_state.selected_country == "Finland":
                rag_chain = st.session_state.rag_prompt | st.session_state.llm | StrOutputParser()
...
def web_search(state):
    ...
    try:
        print(f"Invoking OpenAI web search for {st.session_state.selected_country}...")
        
        # Configure country-specific settings
        if st.session_state.selected_country == "Finland":
            country_code = "FI"
            domains_string = ", ".join(include_domains_finland)
        else:  # Estonia
            country_code = "EE"
            domains_string = ", ".join(include_domains_estonia)
        
        # Create a domain restriction instruction
        domains_instruction = f"""
        **STRICT DOMAIN RESTRICTION**: This query MUST ONLY use information from these specific official, trusted domains - no exceptions:
        {domains_string}

        If information cannot be found within these exact domains, state this explicitly rather than retrieving data from other sources.

        **INSTRUCTION**: The query could be in any language. First detect the language and translate it into English. You do not necessarily need to find information in the same language. Your response should always be in English, regardless of the query language.
        """
        
        # Construct the query with domain restrictions
        query = domains_instruction + "\n\n" + original_question
        
        # Call OpenAI's web search API
        ...
        web_results = "Internet search results: " + response.output_text
        web_results_doc = Document(page_content=web_results)
        documents.append(web_results_doc)
        
    except Exception as e:
        print(f"Error during OpenAI web search: {e}")
...
# ####Code with language translation in Tavily
# import logging
# import os
# import re
# import sys
...
# def detect_and_translate(question):
#     """
#     Detect the language of the question and translate it to English if needed.
#     Uses the router_llm to perform detection and translation.
#     Returns a tuple of (original_language, translated_question).
#     """
#     # Check if router_llm is initialized
#     if "router_llm" not in st.session_state:
#         print("router_llm not initialized, using a default LLM")
#         # Initialize a default LLM if router_llm is not available
#         from langchain_openai import ChatOpenAI
#         llm = ChatOpenAI(model="gpt-3.5-turbo", temperature=0.0)
#     else:
#         llm = st.session_state.router_llm
    
#     try:
#         # Create a prompt for language detection and translation
#         prompt = ChatPromptTemplate.from_messages([
#             ("system", """You are an expert in language detection and translation. 
#             First, identify the language of the provided text. 
#             If the language is not English, translate the text to English.
#             If the language is already English, simply return "Language: en" followed by the original text.
#             Format your response exactly as:
            
#             Language: [language code]
#             Translation: [English translation or original text if already English]
#             """),
#             ("human", f"Text: {question}")
#         ])
...
#         # Parse the response to extract the language code and translation
#         lines = response.strip().split('\n')
#         language_line = lines[0].strip() if len(lines) > 0 else "Language: en"
#         translation_line = lines[1].strip() if len(lines) > 1 else f"Translation: {question}"
        
#         language_code = language_line.replace("Language:", "").strip().lower()
#         translated_question = translation_line.replace("Translation:", "").strip()
        
#         return language_code, translated_question
#     except Exception as e:
#         print(f"Error in language detection and translation: {e}")
#         # Default to English and return the original question if anything fails
#         return "en", question
...
#     language_code, english_question = detect_and_translate(original_question)
#     print(f"Detected language: {language_code}")
    
#     # Use the translated English question for the Tavily search if not English
#     tavily_question = english_question if language_code != "en" else original_question
    
#     # Add country-specific suffix to the question
#     if st.session_state.selected_country == "Estonia":
#         if "estonia" not in tavily_question.lower() and "estonian" not in tavily_question.lower():
#             tavily_question += " in Estonia"
#     else:  # Default to Finland
#         if "finland" not in tavily_question.lower() and "finnish" not in tavily_question.lower():
#             tavily_question += " in Finland"
...
#             print(f"Translated question for Tavily: {tavily_question}")
...
#                 "oph.fi",
#                 "oikeusrekisterikeskus.fi",
#                 "patentochregisterstyrelsen.fi"
#             ]

#         search_result = st.session_state.tavily_client.get_search_context(
#             query=tavily_question,  # Use the translated English question
...
#         # Ensure workflow can continue gracefully
#         documents.append(Document(page_content=f"Web search failed: {e}"))
#     return {"documents": documents, "question": original_question}  # Return original question for the rest of the workflow
...
#####Old code with no language translation in Tavily
# import logging
# import os
# import re
# import sys
